<template>
            ...
            <div class="card card-body mt-4 mb-4">
                <form ref="form" :id="formId" @submit.prevent="submit">
                    ...
                    <div class="form-group">
                        ...
                        <input v-model="updateMovieData.movieName" @keyup="fetchMovies" type="text" class="form-control"
                            id="movieName" :placeholder="props.currentMovie?.movieName" />
                    </div>
                </form>
            </div>
        </div>
    ...
    <div class="list-group mt-4 mb-4">
        <div v-for="movie in movies" @click="selectedMovie(movie)"
            class="list-group-item list-group-item-action card mb-3 user-select-none" data-bs-toggle="list"
            style="max-width: 540px;" :key="movie.id">
            <div class="row g-0">
                <div class="col-md-4">
                    <img :src="`https://image.tmdb.org/t/p/original/${movie.poster_path}`" class="img-fluid rounded-start"
                        loading="lazy" alt="movie poster is missing">
                </div>
                <div class="col-md-8">
                    <div class="card-body">
...
async function fetchMovies(event: KeyboardEvent) {
    (await useTMDBApi()).search.searchMovies({ query: (event.target as HTMLInputElement).value })
        .then((res) => {
