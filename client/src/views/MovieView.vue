  ...
  <div id="movieContainer">
    <div class="container">
      <div class="card card-body mt-4">
        <a
          :href="generateURL(currentMovie?.listId)"
        ...
        >
          <div class="row g-0 mb-4">
            <div class="col-md-4">
              <img
                :src="`https://image.tmdb.org/t/p/original/${tmdbMovieInfo?.poster_path}`"
            ...
            <div class="col-md-8">
              <div class="card-body">
                ...
                <p class="card-text">
                  <small class="text-body-secondary">{{ tmdbMovieInfo?.release_date }}</small>
                </p>
              </div>
            </div>
          </div>
          ...
          <vue-plyr>
            <div data-plyr-provider="youtube" :data-plyr-embed-id="movieTrailer"></div>
          </vue-plyr>
        </div>
        <div v-else>
        ...
        <div>
          ...
          <Modal :close-modal="closeModalEvent" modal-id="editMovie">
            <template v-slot:title> Edit Movie </template>
...
const movieTrailer = computed(() => {
  const trailer = tmdbMovieInfo?.value?.videos?.results?.filter(
...
async function getMovieApiInfo(movieId: number) {
  ;(await useTMDBApi()).endpoint
    .get(`movie/${movieId}`, { append_to_response: 'videos' }) // need to use endpoint to have videos !!
    .then((res) => {
